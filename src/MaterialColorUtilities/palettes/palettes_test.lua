...
local CorePalette = require(script.Parent["core_palette"]).CorePalette
local TonalPalette = require(script.Parent["tonal_palette"]).TonalPalette
describe("TonalPalette", function()
	it("ofBlue", function()
		local blue = TonalPalette:fromInt(0xff0000ff)
...
describe("CorePalette", function()
	it("ofBlue", function()
		local core = CorePalette.of(0xff0000ff)
	...
	it("contentOfBlue", function()
		local core = CorePalette:contentOf(0xff0000ff)
		expect(core.a1:tone(100)).toBe(0xffffffff)
		expect(core.a1:tone(95)).toBe(0xfff1efff)
		expect(core.a1:tone(90)).toBe(0xffe0e0ff)
		expect(core.a1:tone(80)).toBe(0xffbec2ff)
...
describe("KeyColor", function()
	it("Key color with exact chroma", function()
		-- Requested chroma is exactly achievable at a certain tone.
		local palette = TonalPalette:fromHueAndChroma(50.0, 60.0)
		local result = palette.keyColor
		local hueDifference = math.abs(result.hue - 50.0)
		expect(hueDifference).toBeLessThan(10.0)
		local chromaDifference = math.abs(result.chroma - 60.0)
	...
	it("key color with unusually high chroma", function()
		...
		-- the chroma peak.
		local palette = TonalPalette:fromHueAndChroma(149.0, 200.0)
		local result = palette.keyColor
		local hueDifference = math.abs(result.hue - 149.0)
		expect(hueDifference).toBeLessThan(10.0)
	...
	it("key color with unusually low chroma", function()
		...
		local palette = TonalPalette:fromHueAndChroma(50.0, 3.0)
		local result = palette.keyColor
		local hueDifference = math.abs(result.hue - 50.0)
		expect(hueDifference).toBeLessThan(10.0)
		local chromaDifference = math.abs(result.chroma - 3.0)
		expect(chromaDifference).toBeLessThan(0.5)
		local toneDifference = math.abs(result.tone - 50.0)
